...
async def to_greeter(context: RunContext[UserData]) -> Agent:
    ...
    Sends the user to the main appointment agent."""
    appointment_agent = context.userdata.agents.get("MainAgent")  # or "appointment" if that's the key
    if appointment_agent is None:
        return "appointment agent not available."
    await context.session.switch_agent(appointment_agent)
